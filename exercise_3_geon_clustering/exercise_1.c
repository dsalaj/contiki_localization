...
struct filtered_intersect_coords {
    int16_t x;
    int16_t y;
    uint8_t in_circles;
    int32_t sum_dist;
};
struct intersect_coords intersect[180];
    ...
    float cx1, float cy1, float radius1)
{
    ...
    if (dist > radius0 + radius1)
    {
        ...
        int16_t x = (int16_t)((cx0 + cx1) / 2);
        int16_t y = (int16_t)((cy0 + cy1) / 2);
        if (x > 800 || x < -100 || y > 1200 || y < -200) {
            return 0;
        }
    ...
    else if (dist < fabsolute(radius0 - radius1))
    {
        ...
        if (radius0 < radius1) {
            int16_t x = (int16_t)(cx0);
            int16_t y = (int16_t)(cy0);
            if (x > 800 || x < -100 || y > 1200 || y < -200) {
                return 0;
            }
            ...
            int16_t x = (int16_t)(cx1);
            int16_t y = (int16_t)(cy1);
            if (x > 800 || x < -100 || y > 1200 || y < -200) {
                return 0;
            }
...
static void localize() {
    ...
    // calculate in how many circles contain each intersection
    i_counter = 0;
    i_x = 0;
    i_y = 0;
    ...
    // calculate median of number of circles containing all intersections
    //int16_t ccontain_median = median(intersect_counter, ccontain_all);
    int16_t ccontain_median = 0;
    // printf("DRAW_TEXT_NEXT_TO_POINT(%d,%d,Median value of #circles that contain an intersection: %d)\n", -10, -150, ccontain_median);
    // count number of intersections we are interested in (that are contained in more then median circles)
    int16_t num_filtered_intersections = 0;
    for (i_counter = 0; i_counter < intersect_counter; i_counter++){
        int16_t ccontain_diff = intersect[i_counter].in_circles - ccontain_median;
        if (ccontain_diff >= 0) {
            num_filtered_intersections++;
        }
    }
    printf("FILTERED INTERSECTION COUNTER ========================================= %d\n", num_filtered_intersections);
    // copy data to larger data struct for calculating median distances
    struct filtered_intersect_coords fintersect[num_filtered_intersections];
    uint8_t fi_counter = 0;
    for (i_counter = 0; i_counter < intersect_counter; i_counter++){
        //printf("DRAW_CIRCLE(%d,%d,%d,#000000, #000000)\n", intersect[i_counter].x, intersect[i_counter].y, 8);
        int16_t ccontain_diff = intersect[i_counter].in_circles - ccontain_median;
        if (ccontain_diff > 0) {
            fintersect[fi_counter].x = intersect[i_counter].x;
            fintersect[fi_counter].y = intersect[i_counter].y;
            fintersect[fi_counter].in_circles = intersect[i_counter].in_circles;
            fintersect[fi_counter].sum_dist = 0;
            fi_counter++;
        }
    }

    int sum_dist[fi_counter];
    for (i_counter = 0; i_counter < fi_counter; i_counter++){
        int target_x = fintersect[i_counter].x;
        int target_y = fintersect[i_counter].y;
        //printf("DRAW_CIRCLE(%d,%d,%d,#ff0000, #ff0000)\n", fintersect[i_counter].x, fintersect[i_counter].y, 10);
        uint8_t ii_counter = 0;
        uint32_t sum = 0;
        for (ii_counter = 0; ii_counter < fi_counter; ii_counter++){
            int dist = coord_dist(target_x, target_y, fintersect[ii_counter].x, fintersect[ii_counter].y);
            //printf("Distance of %d %d to %d %d is %d\n", target_x, target_y, fintersect[ii_counter].x, fintersect[ii_counter].y, dist);
            sum += (uint32_t)(dist / 20);
        }
        sum_dist[i_counter] = sum;
        //printf("SUM_DIST[%d] = %d\n", i_counter, sum_dist[i_counter]);
    }
    uint8_t count_use = 0;
    int inter_median_dist = (int)(median(num_filtered_intersections, sum_dist));
    printf("INTER_MEDIAN_DIST = %d\n", inter_median_dist);
    for (i_counter = 0; i_counter < fi_counter; i_counter++){
        int target_x = fintersect[i_counter].x;
        int target_y = fintersect[i_counter].y;
        //if (sum_dist[i_counter] < (inter_median_dist - (int)(inter_median_dist/20))) {
        if (sum_dist[i_counter] < (inter_median_dist * 1.2)) {
            count_use++;
            //printf("DRAW_TEXT_NEXT_TO_POINT(%d,%d,%d)\n", target_x, target_y, (int)(sum_dist[i_counter]/fi_counter));
            printf("DRAW_CIRCLE(%d,%d,%d,#00ff00, #00ff00)\n", fintersect[i_counter].x, fintersect[i_counter].y, 8);
            i_x = i_x + intersect[i_counter].x;
            i_y = i_y + intersect[i_counter].y;
        } else {
            //printf("DRAW_TEXT_NEXT_TO_POINT(%d,%d,%d)\n", target_x, target_y, (int)(sum_dist[i_counter]/fi_counter));
    ...
    int cx = i_x / count_use;
    int cy = i_y / count_use;
    printf("DRAW_CIRCLE(%d,%d,%d,#aaaa00, #50ffa0)\n", cx, cy, 20);
    printf("DRAW_TEXT_NEXT_TO_POINT(%d,%d,CLUSTER.AVG.)\n", cx, cy);


    intersect_counter = 0;
    my_coords.x = (cx + mmx)/2;
    my_coords.y = (cy + mmy)/2;
    if (my_coords.x < 315) {
        my_coords.x -= 50;
    } else {
        my_coords.x += 50;
